classdef (Abstract) BaseClient < handle & matlab.mixin.CustomDisplay
    % BASECLIENT Base class for RESTful macrobond services.
    ...
    methods
        function obj = BaseClient(options)
            % macrobond.BaseClient constructor to be called from 
            ...
            if ~isfield(options,"configFile") && isfile(which("macrobond.Client.Settings.json"))
                options.configFile = which("macrobond.Client.Settings.json");
            end
    ...
    methods (Access=protected)
        ...
        function  [request, httpOptions, uri] = requestAuth(obj, authNames, request, httpOptions, uri)
            ...
            switch authName
                ...
                case "auth"
                    % oAuth authentication, calls getOAuthToken and adds returned token as Bearer authorization header.
                    request.Header(end+1) = matlab.net.http.field.AuthorizationField("Authorization","Bearer " + obj.getOAuthToken("auth"));
                otherwise
                    error("macrobond:UnknownAuthorization", "Operation requested an authentication method which was not specified in the OpenAPI spec.")
            end
        end

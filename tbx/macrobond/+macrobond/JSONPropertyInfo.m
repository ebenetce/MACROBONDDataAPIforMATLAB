classdef JSONPropertyInfo < handle
    % JSONPROPERTYINFO class used by macrobond.JSONMapper internally
    ...
    properties
        ...
        discriminatorInfo macrobond.JSONDiscriminator
    ...
    methods (Static)
        function props = getPropertyInfo(obj)
            ...
            ps = properties(obj);
            N = length(ps);
            props = macrobond.JSONPropertyInfo.empty(N,0);
            for i = 1:N
                ...
                if isempty(attrs) % If there are no attributes
                    ...
                    fi = find(contains(attrs,"macrobond.JSONMapper.fieldName"));
                    if isempty(fi) 
                    ...
                    fi = find(contains(attrs,"macrobond.JSONMapper.discriminator"));
                    if ~isempty(fi) 
                ...
                props(i).doNotDecode = any(strcmp(attrs,'macrobond.JSONMapper.doNotDecode'));
            end
        end
    end

classdef (Abstract) JSONMapper < handle
    % macrobond.JSONMapper base class - adds JSON serialization and deserialization.
    ...
    properties (Access=private)
        MATLABProperties macrobond.JSONPropertyInfo
    end
    ...
    methods (Static)
        ...
        function fn = fieldName(~,fn)
            % FIELDNAME macrobond.JSONMapper Annotation
            ...
            %
            %   properties
            %       some_field {macrobond.JSONMapper.fieldName(some_field,"some.field")}
            %   end
            %
        end

        function JSONArray(~)
            % JSONARRAY macrobond.JSONMapper Annotation
        ...
        function info = discriminator(~,Value,Class)
            % DISCRIMINATOR macrobond.JSONMapper Annotation
            ...
            info = macrobond.JSONDiscriminator(Value,Class);
        end

        function out = epochDatetime(in,options)
            % EPOCHDATETIME macrobond.JSONMapper Annotation
            ...
            %       start_date {macrobond.JSONMapper.epochDatetime}
            %       % end_date is UNIX timestamp in milliseconds
            %       end_date {macrobond.JSONMapper.epochDatetime(end_date,'TicksPerSecond',1000)}
            %   end
            arguments
                in
                options.TimeZone = '' 
            ...
            if isstring(in) % From JSON to datetime
                ...
                try
                    ...
                    fprintf(2,'Error in macrobond.JSONMapper: %s\n\n',ME.message);
                end                    
        ...
        function out = stringDatetime(in,format,options)
            % STRINGDATETIME macrobond.JSONMapper Annotation
            % When working with datetime fields either epochDatetime or
            % stringDatetime annotation is required to specify how the
            % datetime is encoded in JSON. This must be the first
            % annotation.
            ...
            %
            %   properties
            %       start_date {macrobond.JSONMapper.stringDatetime(start_date,'yyyy-MM-dd''T''HH:mm:ss')}
            %   end
            arguments
                in
                format string
        ...
        function ConstructorArgument(arg)
            ...
            % constructor to be used when working with nested macrobond.JSONMapper
            % derived classes.
    ...
    methods
        function obj = JSONMapper(s,inputs)
            % macrobond.JSONMapper Constructor. It is possible to call this from
        ...
        function obj = initialize(obj,s,inputs)
            ...
            obj.MATLABProperties = macrobond.JSONPropertyInfo.getPropertyInfo(obj);
        ...
        function obj = fromJSON(obj,json,calledFromConstructor)
            ...
            try
                ...
                % in the hierarchy derive from macrobond.JSONMapper.
                %
                % Example:
                %   obj = myClass;
        ...
        function json = jsonencode(obj,raw)
                ...
                for currProp = obj(arrayIndex).MATLABProperties
                    ...
                    switch currProp.dataType
                        ...
                        case {?uint64}
                            ...
                            jObject.add(currProp.jName,macrobond.JSONMapper.MATLABGSON.toJsonTree(val));
                        case {?containers.Map}
                            vals = obj(arrayIndex).(currProp.mName).values;
                            keys = obj(arrayIndex).(currProp.mName).keys;
                            m = com.google.gson.JsonObject;
                            for i=1:length(vals)
                                m.add(keys{i},macrobond.JSONMapper.MATLABGSON.toJsonTree(vals{i}));
                            end
                            jObject.add(currProp.jName,m);
                        case {?meta.class, ?macrobond.JSONMapperMap} % free form
            ...
            if isArray
                if raw
                    ...
                    json = char(macrobond.JSONMapper.MATLABGSON.toJson(arr));
                end
            else
                if raw
                    json = jObject;
                else
                    json = char(macrobond.JSONMapper.MATLABGSON.toJson(jObject));
                end
            end
        end
...
function out = getJSONScalarOrArray(val,forceArray)
    ...
    if forceArray && ~isa(val,'com.google.gson.JsonArray') && length(val) == 1 %#ok<ISCL>
        ...
        out.add(macrobond.JSONMapper.MATLABGSON.toJsonTree(val));
    else
        out = macrobond.JSONMapper.MATLABGSON.toJsonTree(val);
    end
end
...
function val = getScalarOrArray(curVal,type,options)
    ...
    if options.doNotDecode
        ...
        val = macrobond.JSONMapper.MATLABGSON.fromJson(curVal,t);
    end
    if type == "string"
        val = string(val);
    end

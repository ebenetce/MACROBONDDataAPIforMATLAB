classdef (Abstract) JSONMapper < handle
    % MacrobondMATLABAPI.JSONMapper base class - adds JSON serialization and deserialization.
    ...
    % call the MacrobondMATLABAPI.JSONMapper constructor from their constructor:
    % 
    ...
    % MacrobondMATLABAPI.JSONMapper.
    %
    % Annotations can be added to properties as "validation functions".
    %
    % MacrobondMATLABAPI.JSONMapper Methods:
    %
    ...
    properties (Access=private)
        MATLABProperties MacrobondMATLABAPI.JSONPropertyInfo
    end
    ...
    methods (Static)
        function fn = fieldName(~,fn)
            % FIELDNAME MacrobondMATLABAPI.JSONMapper Annotation
            ...
            %
            %   properties
            %       some_field {MacrobondMATLABAPI.JSONMapper.fieldName(some_field,"some.field")}
            %   end
            %
        end
        ...
        function JSONArray(~)
            % JSONARRAY MacrobondMATLABAPI.JSONMapper Annotation
        ...
        function out = epochDatetime(in,options)
            % EPOCHDATETIME MacrobondMATLABAPI.JSONMapper Annotation
            ...
            %       start_date {MacrobondMATLABAPI.JSONMapper.epochDatetime}
            %       % end_date is UNIX timestamp in milliseconds
            %       end_date {MacrobondMATLABAPI.JSONMapper.epochDatetime(end_date,'TicksPerSecond',1000)}
            %   end
            arguments
                in
                options.TimeZone = '' 
            ...
            if isstring(in) % From JSON to datetime
                ...
                try
                    ...
                    fprintf(2,'Error in MacrobondMATLABAPI.JSONMapper: %s\n\n',ME.message);
                end                    
        ...
        function out = stringDatetime(in,format,options)
            % STRINGDATETIME MacrobondMATLABAPI.JSONMapper Annotation
            % When working with datetime fields either epochDatetime or
            % stringDatetime annotation is required to specify how the
            % datetime is encoded in JSON. This must be the first
            % annotation.
            ...
            %
            %   properties
            %       start_date {MacrobondMATLABAPI.JSONMapper.stringDatetime(start_date,'yyyy-MM-dd''T''HH:mm:ss')}
            %   end
            arguments
                in
                format string
        ...
        function ConstructorArgument(arg)
            ...
            % constructor to be used when working with nested MacrobondMATLABAPI.JSONMapper
            % derived classes.
    ...
    methods
        function obj = JSONMapper(s,inputs)
            % MacrobondMATLABAPI.JSONMapper Constructor. Call this from
            ...
            obj.MATLABProperties = MacrobondMATLABAPI.JSONPropertyInfo.getPropertyInfo(obj);
        ...
        function obj = fromJSON(obj,json)
            try
                ...
                % in the hierarchy derive from MacrobondMATLABAPI.JSONMapper.
                %
                % Example:
                %   obj = myClass;
                    ...
                    for currProp = obj(1).MATLABProperties
                        ...
                        if curElement.has(currProp.jName) && ~curElement.get(currProp.jName).isJsonNull
                            ...
                            switch currProp.dataType
                                ...
                                case {?MacrobondMATLABAPI.JSONMapperMap}
                                    map = MacrobondMATLABAPI.JSONMapperMap;
                                    it = curVal.entrySet.iterator;
                                    while it.hasNext
                                        kv = it.next;
                                        map(char(kv.getKey)) = char(kv.getValue.getAsString());
        ...
        function json = jsonencode(obj,raw)
                ...
                for currProp = obj(arrayIndex).MATLABProperties
                    ...
                    switch currProp.dataType
                        ...
                        case {?uint64}
                            ...
                            jObject.add(currProp.jName,MacrobondMATLABAPI.JSONMapper.MATLABGSON.toJsonTree(val));
                        case {?containers.Map}
                            vals = obj(arrayIndex).(currProp.mName).values;
                            keys = obj(arrayIndex).(currProp.mName).keys;
                            m = com.google.gson.JsonObject;
                            for i=1:length(vals)
                                m.add(keys{i},MacrobondMATLABAPI.JSONMapper.MATLABGSON.toJsonTree(vals{i}));
                            end
                            jObject.add(currProp.jName,m);
                        case {?meta.class, ?MacrobondMATLABAPI.JSONMapperMap} % free form
            ...
            if isArray
                if raw
                    ...
                    json = char(MacrobondMATLABAPI.JSONMapper.MATLABGSON.toJson(arr));
                end
            else
                if raw
                    json = jObject;
                else
                    json = char(MacrobondMATLABAPI.JSONMapper.MATLABGSON.toJson(jObject));
                end
            end
        end
...
function out = getJSONScalarOrArray(val,forceArray)
    ...
    if forceArray && ~isa(val,'com.google.gson.JsonArray') && length(val) == 1
        ...
        out.add(MacrobondMATLABAPI.JSONMapper.MATLABGSON.toJsonTree(val));
    else
        out = MacrobondMATLABAPI.JSONMapper.MATLABGSON.toJsonTree(val);
    end
end
...
function val = getScalarOrArray(curVal,type)
    ...
    val = MacrobondMATLABAPI.JSONMapper.MATLABGSON.fromJson(curVal,t);
    if type == "string"
        val = string(val);
    end

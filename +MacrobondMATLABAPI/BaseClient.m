classdef (Abstract) BaseClient < handle & matlab.mixin.CustomDisplay
    % BASECLIENT Base class for RESTful MacrobondMATLABAPI services.
    ...
    properties (Constant)
        ...
        cookies = MacrobondMATLABAPI.CookieJar(fileparts(mfilename('fullpath')));
    end

    methods
        function obj = BaseClient(options)
            % MacrobondMATLABAPI.BaseClient constructor to be called from 
            ...
            if ~isfield(options,"configFile") && isfile(which("MacrobondMATLABAPI.Client.Settings.json"))
                options.configFile = which("MacrobondMATLABAPI.Client.Settings.json");
            end
    ...
    methods (Access=protected)
        ...
        function  [request, httpOptions, uri] = requestAuth(obj, authNames, request, httpOptions, uri)
            ...
            switch authName
                case "ClientDirectAccess"
                    ...
                    request.Header(end+1) = matlab.net.http.field.AuthorizationField("Authorization","Bearer " + obj.getOAuthToken("ClientDirectAccess"));
                otherwise
                    error("MacrobondMATLABAPI:UnknownAuthorization", "Operation requested an authentication method which was not specified in the OpenAPI spec.")
            end
        end
        ...
        function token = getOAuthToken(obj, name) %#ok<INUSD>
            ...
            %         ];
            %     otherwise
            %         error("MacrobondMATLABAPI:UnknownOAuth", "Operation requested an OAUth flow which was not specified in the OpenAPI spec.")
            % end
        end

classdef JSONPropertyInfo < handle
    % JSONPROPERTYINFO class used by MacrobondMATLABAPI.JSONMapper internally
    ...
    methods (Static)
        function props = getPropertyInfo(obj)
            ...
            ps = properties(obj);
            N = length(ps);
            props = MacrobondMATLABAPI.JSONPropertyInfo.empty(N,0);
            for i = 1:N
                ...
                if isempty(attrs) % If there are no attributes
                    ...
                    props(i).isArray = any(strcmp(attrs,'MacrobondMATLABAPI.JSONMapper.JSONArray'));
                    % Check for fieldName attribute
                    fi = find(contains(attrs,"MacrobondMATLABAPI.JSONMapper.fieldName"));
                    if isempty(fi) 
